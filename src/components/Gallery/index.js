...
class Gallery extends Component {
  state = {selectedThumbnail: imagesList[0]}

  getSelectedImage = id => {
    const selectedImage = imagesList.filter(eachItem => id === eachItem.id)
    this.setState({selectedThumbnail: selectedImage[0]})
  }
        ...
        <ul className="thumbnail-container">
          {imagesList.map(eachItem => (
            <ThumbnailItem
              ...
              key={eachItem.id}
              isActive={eachItem.imageUrl === selectedThumbnail.imageUrl}
            />
          ))}
        </ul>
      </div>
